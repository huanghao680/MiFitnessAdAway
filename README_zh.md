...
- **试用表盘自动导出（实验）** —— 试用下载完成后，缓存的 `resource.bin` 按「12→19」等长换新 ID，以表盘中文名写入，供第三方软件导入（已在小米手环 10 Pro + AstroBox 验证）。导出位置可在 设置 → 表盘 → 「导出位置」里自定义：填相对路径，如 `Download/表盘导出`、`Documents/2026/面孔`（首级目录仅限 `Download` / `Documents`，这是 MediaStore 对第三方 App 的全部放行范围，其他会回退 `Download`）。改位置时会问是否把已导出的表盘重导过去，且去重按位置判定，保证新目录真的能收到文件。导出 ID 会从服务端清理名单中摘除，第三方刷入的表盘同步不再被删；已导出缓存下次扫描即清理（快照 + 交接/推送保护）。每次扫描经 Toast/通知告知结果（含真实落盘位置）。
