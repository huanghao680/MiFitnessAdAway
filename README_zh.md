...
- **试用表盘自动导出（实验）** —— 试用下载完成后，先在缓存目录里定位该表盘的包体：老版 App 固定命名为 `resource.bin`，新版 App 以内容 MD5 命名（32 位十六进制、无扩展名）；名字对不上时按包体头部里的表盘 ID 兜底匹配（上游逻辑已在小米手环 10 Pro + AstroBox 验证，小米手环 8 Pro 等走新版命名的机型由兜底匹配覆盖）。定位到包体后按「12→19」等长换新 ID，以表盘中文名写入，供第三方软件导入。导出位置可在 设置 → 表盘 → 「导出位置」里自定义：填相对路径，如 `Download/表盘导出`、`Documents/2026/面孔`（首级目录仅限 `Download` / `Documents`，这是 MediaStore 对第三方 App 的全部放行范围，其他会回退 `Download`）。改位置时会问是否把已导出的表盘重导过去，且去重按位置判定，保证新目录真的能收到文件。导出 ID 会从服务端清理名单中摘除，第三方刷入的表盘同步不再被删；已导出缓存下次扫描即清理（快照 + 交接/推送保护）。每次扫描经 Toast/通知告知结果（含真实落盘位置）。
